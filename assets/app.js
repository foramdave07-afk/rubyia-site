...
function uniqueSorted(arr, key){
  const s = new Set();
  arr.forEach(o=>{ const v=(o[key]||'').trim(); if(v) s.add(v); });
  return Array.from(s).sort((a,b)=>a.localeCompare(b));
}

function populateFilters(){
  const groups = uniqueSorted(DATA, 'group');
  const brands = uniqueSorted(DATA, 'brand');
  const countries = uniqueSorted(DATA, 'country');
...
function sortView(list){
  const by = $('f_sort').value;
  const key = by==='name' ? 'name' : by;
...
function cardHTML(r, idx){
  const chips = [r.alias?`Code: ${r.alias}`:'', r.unit?`Unit: ${r.unit}`:'', r.brand||'', r.country||'', r.material||''].filter(Boolean);
...
function renderMore(){
  ...
  slice.forEach((r,i)=>{
    const idx = CURSOR + i; // index within VIEW
...
function updateSelectedCount(){
  elSelCount.textContent = String(SELECTED.size);
  $('btn_add_selected').disabled = SELECTED.size===0;
}
...
document.addEventListener('DOMContentLoaded', async ()=>{
  ...
  // Pagination
  elMore.addEventListener('click', renderMore);

  // Select mode toggle
  $('toggle_select').addEventListener('change', (e)=> toggleSelectMode(e.target.checked));

  // Grid click handlers (event delegation)
  elGrid.addEventListener('click', (e)=>{
    ...
    if(t.classList.contains('add')){
      ...
      addOne(VIEW[idx]);
      t.textContent = 'Added';
      setTimeout(()=> t.textContent='Add to Enquiry', 900);
    }
    if(t.classList.contains('pick')){
      const idx = Number(t.dataset.idx);
      if(t.checked){ SELECTED.set(idx, VIEW[idx]); } else { SELECTED.delete(idx); }
      updateSelectedCount();
    }
  });

  // Because checkboxes are inside <h3>, clicks may bubble from the h3 — handle change too
