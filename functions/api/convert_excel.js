...
export async function onRequestPost({ request, env }) {
  try {
    ...
    const apiKey = env.API2PDF_KEY;
    if (!apiKey) return new Response('Missing API2PDF_KEY', { status: 500 });
    ...
    return new Response(pdfBuf, { headers: { 'Content-Type': 'application/pdf', 'Content-Disposition': `attachment; filename=\"${(file.name or 'quotation')}.pdf\"` } });
